...
using BaseServiceCore.Model.Generate;
using SqlSugar.IOC;

namespace BaseServiceCore.SqlSugar
{
    /// <summary>
    /// 初始化表
    /// </summary>
    public class InitTable
    {
        ...
        public static void InitDb()
        {
            ...
            //Extend Entity
            db.CodeFirst.InitTables(typeof(Chapeter));
            db.CodeFirst.InitTables(typeof(Course));
            db.CodeFirst.InitTables(typeof(Examquestion));
            db.CodeFirst.InitTables(typeof(Judge));
            db.CodeFirst.InitTables(typeof(Select));
            db.CodeFirst.InitTables(typeof(UserScore));
            db.CodeFirst.InitTables(typeof(Exam));
            //db.CodeFirst.InitTables(typeof(UserOnlineLog));
        }
        public static void InitNewTb()
        {
            var db = DbScoped.SugarScope;
            var t1 = db.DbMaintenance.IsAnyTable(nameof(UserOnlineLog));
            if (!t1)
            {
                db.CodeFirst.InitTables(typeof(UserOnlineLog));
            }
