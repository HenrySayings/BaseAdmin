			...
			<view class="content">
				<view class="action-bar h-flex-x">
					<view @tap="addFavorite">
						<view class="h-flex-x">
							<button style="padding-left: 10rpx;" @click="HandleCommit">立即提交</button>
						</view>
					</view>
...
<script>
	...
	import req from '../apis/select.js';
	import q from "../apis/score.js"
	export default {
		components: {uniIcons},
		data() {
			return {
		...
		methods: {
			...
			HandleCommit() {
			    let data = {
			        "examId": "12BFFD23-264E-455D-941C-232A83CC2163",
			        "Score": this.count,
			        "isDeletedLabel": ""
			    };
			    q.Examcommit(data).then(res => {
			        uni.showToast({
			            title: '提交成功，您的成绩为:' + this.count,
			            icon: 'success',
			            duration: 3000
			        }).then(() => { // 注意这里
			        });
