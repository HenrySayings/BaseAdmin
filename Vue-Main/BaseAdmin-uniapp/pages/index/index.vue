...
<template>
	<view class="container">
		<!-- <input type="text" placeholder="请输入搜索的内容" value="" /> -->
		<view class="title">
			<view :class="{active:index==current}" @click="choice(index)" v-for="(item,index) in title" :key="index">
				{{item}}</view>
		</view>
		<swiper :indicator-active-color="indicator_active_color" :circular="circular" :interval="interval"
			:autoplay="autoplay" :indicator-dots="indicator_dots" class="pictrue">
			<swiper-item>
		...
		<view class="advance">
			<view @click="HandleExam">java全栈从后端走向全栈</view>
			<view @click="HandleExam()">大前端前端高级进阶</view>
			<view @click="HandleExam()">java架构师专家系统养成</view>
		</view>
		<view class="hot_free">
			<text>热门免费</text>
			<text>0门槛开启你的高薪之路</text>
		</view>
		<view class="free_curriculum" v-for="v in courseItem" @click="to_video(v.courseId)">
			<image :src="v.picUrl" mode=""></image>
			<view class="">{{v.courseName}}</view>
...
<script>
	import req from '../apis/course.js'
	export default {
		data() {
			return {
				title: ["推荐", "热门", "在线", "专栏"],
		...
		onLoad() {
			req.GetUniCourses().then(res => {
				this.courseItem = res.data;
			});
		}
	}
</script>